...
import common.toolkit.java.entity.constant.SymbolConstant;
...
public class ThreadUtil {
    ...
    public static void startThread(Runnable runnable, String threadName, int threadNum, long sleepTime) {
        ...
        for (int i = 0; i < threadNum; i++) {
            Thread thread = new Thread(runnable, SymbolConstant.POUND + StringUtil.defaultIfBlank(threadName, "Thread")
                    + SymbolConstant.MINUS_SIGN + i);
            thread.start();
        }
        try {
     ...
     * @throws IllegalParamException
     */
    public static void scheduleAtFixedRateDelayDaysHour(TimerTask task, int delayDays, int hourOfTomorrow, long period)
            throws IllegalParamException, Exception {
        ThreadUtil.scheduleAtFixedRateDelayTimeMillisDelay(task,
                DateUtil.getTimeMillisToAfterDaysHour(delayDays, hourOfTomorrow), period);
    }
