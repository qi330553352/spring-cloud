...
import common.toolkit.java.entity.util.io.NetUtil;
...
public class JmxBean {
    ...
    public JmxBean(String ip, String port, String jndiPath) {
        this.port     = port;
        this.ip       = ip;
    ...
    public MBeanServerConnection getMBeanServerConnection() throws Throwable {
        if (null == connector) {
            try {
            	JMXServiceURL address = new JMXServiceURL("service:jmx:rmi:///jndi/rmi://" + ip + ":" + port + "/" + jndiPath);
